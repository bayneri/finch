...
class APIClient {
  ...
  fileprivate class func getHeader() -> [String: String]? {
    if let session = SessionManager.currentSession {
      return baseHeaders + [
        HTTPHeader.token.rawValue: session.token ?? ""
      ]
    }
    return baseHeaders
  }
  ...
  class func requestJson(_ method: HTTPMethod, url: String, params: [String: Any]? = nil, paramsEncoding: ParameterEncoding? = nil, success:@escaping (_ data:JSON ) -> () = { data in}, failure: @escaping (_ error:Error ) -> () = { error in}) {
    let encoding = paramsEncoding ?? defaultEncoding(forMethod: method)
    let header = APIClient.getHeader()
    let requestUrl = getBaseUrl() + url
    Alamofire.request(requestUrl, method: method, parameters: params, encoding: encoding, headers: header)
